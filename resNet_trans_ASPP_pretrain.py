...
class SaliencyResNetTransPretrained(nn.Module):
    ...
    def __init__(
        ...
        # Projection layers
        self.bottleneck_proj = nn.Conv2d(2048, 384, kernel_size=1)
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, 1200, 384))
        nn.init.trunc_normal_(self.pos_embed, std=0.02)

        self.transformer = nn.Sequential(
            TransformerBlock(dim=384, num_heads=6),
            TransformerBlock(dim=384, num_heads=6),
        )
    ...
    def _get_pos_embed(self, Ht, Wt):
        ...
        pos = self.pos_embed.transpose(1, 2).reshape(1, 384, old_H, old_W)
    ...
    def forward(self, x):
        ...
        x = self.stage5(s4) 
        x = self.aspp(x)        # 2048 channels, same spatial size as s4 due dilation
